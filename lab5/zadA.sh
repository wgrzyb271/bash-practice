...
#
file='dodatkowe/listaSluchaczy_E07-19x.csv'

# zadanie
#tail -n +2 "$file" | cut -d ',' -f 2 | rev | cut -c1-6 | rev | sed 's/$/@student.pwr.edu.pl/'

tail -n +14 "$file" | cut -d ';' -f 2 | tr -d '_' | rev | cut -c1-6 | rev | sed 's/$/@student.pwr.edu.pl/' | head -n -1
