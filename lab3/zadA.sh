...
# sprawdzenie, czy podane argumenty są poprawne
if [ $# -ne 2 ] || [ ! -d "$dir" ] || [ ! -e "$file" ]; then
    exit 1
fi

# pobranie i-wezla pliku
inode=$(stat -c '%i' "$file")

# wyszukiwanie wszystkich dowiazan twardych do wskazanego pliku
find "$dir" -maxdepth 1 -type f -links +1 -exec stat --format='%i %n' {} \; | grep "^$inode " | cut -d ' ' -f 2 | cat
