...
# Utworzenie dowiązania symbolicznego w katalogu ddd do wszystkich plików wykonywalnych w katalogu aaa
for file in "$FROM"/*;
do
    # Sprawdzenie, czy obiekt jest plikiem i jest wykonywalny
	...
	if [ ! -e "$target" ]; then	
       		 ...
       		 ln -s ../"$file" "$target"
       		 echo "Utworzono dowiązanie do: $name"
	fi		 
    fi
done
